...
struct LaunchConfiguration {
	...
	QString workingDirectory() const {
		return mConfigGroup.readEntry("workingdirectory", QString());
	}
	...
	void setName(const QString& newName) { mConfigGroup.writeEntry("name", newName); }
	void setModule(const QString& newModule) { mConfigGroup.writeEntry("module", newModule); }
	void setWorkingDirectory(const QString& newWd) {
		mConfigGroup.writeEntry("workingdirectory", newWd);
	}
	void setArguments(const QString& newArgs) { mConfigGroup.writeEntry("arguments", newArgs); }
	void setEnv(const QStringList& newEnv) { mConfigGroup.writeEntry("environment", newEnv); }
...
public:
	...
	LaunchConfiguration configByName(const QString& str) {
		for (const auto& config : configurations()) {
			if (config.name() == str) { return config; }
		}
		return {};
	}
