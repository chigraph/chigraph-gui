...
public:
	ChiItemSelectionDialog(chi::Context& ctx, boost::filesystem::path* toFill, WorkspaceTree::eType type, QWidget* parent = nullptr);
	
	void setCurrentItem(const boost::filesystem::path& newItem);
	
	static boost::filesystem::path getItem(QWidget* parent, chi::Context& ctx, const QString& title, WorkspaceTree::eType type, const boost::filesystem::path& startWithSelection);
