...
ModulePropertiesDialog::ModulePropertiesDialog(QWidget* parent, chi::GraphModule& modToEdit) : QDialog{parent}, mModule{&modToEdit} {
	...
	// create new button
	auto newDepButton = new QPushButton{QIcon::fromTheme(QStringLiteral("gtk-add")), i18n("New Dependency")};
	...
	connect (newDepButton, &QPushButton::clicked, this, [this, depsList] (bool) {
		...
		boost::filesystem::path newDep = ChiItemSelectionDialog::getItem(this, mModule->context(), i18n("Select new dependency"), WorkspaceTree::MODULE);
		
		if (newDep.empty()) {
			return;
		}
		
		if (mModule->dependencies().count(newDep) != 0) {
			return;
		}
		...
		if (!res) {
			KMessageBox::detailedError(this, i18n("Failed to add dependency for module"), QString::fromStdString(res.dump()));
	...
	connect(checkBox, &QCheckBox::stateChanged, this, [this](int newState) {
		switch (newState) {
		case Qt::Unchecked:
			mModule->setCEnabled(false);
			return;
		case Qt::Checked:
			mModule->setCEnabled(true);
			return;
		default: return;
		}
	});
