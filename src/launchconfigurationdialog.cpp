...
LaunchConfigurationDialog::LaunchConfigurationDialog(LaunchConfigurationManager& manager)
	...
	auto leftWidget = new QWidget;
	{
		...
		// populate it
		for (const auto& config : manager.configurations()) { mConfigList->addItem(config.name()); }
		connect(mConfigList, &QListWidget::itemDoubleClicked, this,
		        [this](QListWidgetItem* item) { selectConfig(item->text()); });
	}
	...
	auto rightWidget = new QWidget;
	{
		...
		// name
		{
			...
			connect(mNameEdit, &QLineEdit::textChanged, this,
			        &LaunchConfigurationDialog::nameChanged);
