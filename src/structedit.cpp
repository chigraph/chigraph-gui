...
void StructEdit::makeGUI() {
	...
	if (auto oldLay = layout()) { deleteLayout(oldLay); }
	...
	connect(newButton, &QAbstractButton::clicked, this, [this](bool) {
		mStruct->addType(mStruct->context().langModule()->typeFromName("i32"), {},
		                 mStruct->types().size());
...
void StructEdit::refreshReferences() {
	...
	auto allInstances =
	    ctx.findInstancesOfType(mStruct->module().fullNamePath(), "_make_" + mStruct->name());
	auto breakInstances =
	    ctx.findInstancesOfType(mStruct->module().fullNamePath(), "_break_" + mStruct->name());
	...
	for (auto inst : allInstances) {
		...
		if (view != nullptr) { view->refreshGuiForNode(*inst); }
	}
}
